     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 while (!queue.isEmpty() && count < numCompletions) {
    		 TrieNode first = queue.remove(0);
    		 if (first.endsWord()) {
    			 results.add(first.getText());
    			 count++;
    		 }
    		 List<Character> cs= new LinkedList<Character>(first.getValidNextCharacters());
    		 for (int i = 0; i < cs.size(); i++) {
    			 char c = cs.get(i);
    			 queue.add(first.getChild(c));
    		 }
    	 }
         return results;
     }
