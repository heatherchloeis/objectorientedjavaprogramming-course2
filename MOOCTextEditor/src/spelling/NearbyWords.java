...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		HashSet<String> duplicates = new HashSet<String>();   // to avoid exploring the same  
														   // string multiple times
		List<String> results = new LinkedList<String>();   // words to return
		// insert first node
		queue.add(word);
		duplicates.add(word);
		int count = 0;
		...
		while (!queue.isEmpty() && count < numSuggestions && max < THRESHOLD) {
			String curr = queue.remove(0);
			List<String> disOne = distanceOne(curr, true);
